import PomodoroTimerPlugin from 'main'
import { type CachedMetadata, type TFile } from 'obsidian'
import { extractTaskComponents } from 'utils'
import { writable, type Readable, type Writable } from 'svelte/store'

import type { Unsubscriber } from 'svelte/motion'
import { DESERIALIZERS } from 'serializer'
import { sotaLog } from './Debug' // <--- ADICIONADO

export const DEFAULT_TASK: TaskItem = {
    actual: 0, expected: 0, path: '', fileName: '', text: '', name: '', status: '',
    blockLink: '', checked: false, done: '', due: '', created: '', cancelled: '',
    scheduled: '', start: '', description: '', priority: '', tags: [], line: -1,
};

export type TaskItem = {
    path: string
    text: string
    fileName: string
    name: string
    status: string
    blockLink: string
    checked: boolean
    done: string
    due: string
    created: string
    cancelled: string
    scheduled: string
    start: string
    description: string
    priority: string
    expected: number
    actual: number
    tags: string[]
    line: number
}

export type TaskStore = {
    list: TaskItem[]
}

export default class Tasks implements Readable<TaskStore> {
    private plugin: PomodoroTimerPlugin

    private _store: Writable<TaskStore>

    public subscribe

    private unsubscribers: Unsubscriber[] = []

    private state: TaskStore = {
        list: [],
    }

    public static getDeserializer(format: string) {
        return DESERIALIZERS[format]
    }

    constructor(plugin: PomodoroTimerPlugin) {
        this.plugin = plugin

        this._store = writable(this.state)

        this.unsubscribers.push(
            this._store.subscribe((state) => {
                this.state = state
            }),
        )

        this.subscribe = this._store.subscribe
    }

    public loadFileTasks(file: TFile | null) {
        sotaLog('Tasks', 'loadFileTasks: Função chamada.', { filePath: file?.path }); // <--- LOG E

        if (file && file.extension == 'md') {
            this.plugin.app.vault.cachedRead(file).then((c) => {
                sotaLog('Tasks', 'loadFileTasks: Arquivo lido. Chamando resolveTasks...'); // <--- LOG F
                let tasks = resolveTasks(
                    'TASKS', // <-- HARDCODED para o nosso padrão SOTA
                    file,
                    c,
                    this.plugin.app.metadataCache.getFileCache(file),
                )
                sotaLog('Tasks', 'loadFileTasks: Resultado de resolveTasks:', { taskCount: tasks.length, tasks }); // <--- LOG G
                this._store.update(() => ({
                    list: tasks,
                }))
            })
        } else {
            sotaLog('Tasks', 'loadFileTasks: Arquivo nulo ou não-markdown. Limpando a lista.'); // <--- LOG H
            this._store.update(() => ({
                list: [],
            }))
        }
    }

    public updateTasks(tasks: TaskItem[]) {
        this._store.update((state) => {
            state.list = tasks;
            return state;
        });

        // sync active task
        if (this.plugin.tracker?.task?.blockLink) {
            let task = tasks.find(
                (item) =>
                    item.blockLink &&
                    item.blockLink ===
                        this.plugin.tracker?.task?.blockLink,
            );
            if (task) {
                this.plugin.tracker.sync(task);
            }
        }
    }

    public updateTaskState(taskToUpdate: TaskItem, newCheckedState: boolean) {
        this._store.update(currentStore => {
            const taskIndex = currentStore.list.findIndex(t => t.path === taskToUpdate.path && t.line === taskToUpdate.line);
            if (taskIndex !== -1) {
                currentStore.list[taskIndex].checked = newCheckedState;
            }
            return currentStore; // Retorna o store mutado para acionar a reatividade do Svelte
        });
    }

    public clearTasks() {
        this._store.update(() => ({
            list: [],
        }))
    }

    public destroy() {
        for (let unsub of this.unsubscribers) {
            unsub()
        }
    }
}

export function resolveTasks(
    format: string,
    file: TFile,
    content: string,
    metadata: CachedMetadata | null,
): TaskItem[] {
    // ... (o resto da função permanece o mesmo)
    if (!content || !metadata) {
        return []
    }

    let cache: Record<number, TaskItem> = {}
    const lines = content.split('\n')
    for (let rawElement of metadata.listItems || []) {
        if (rawElement.task) {
            let lineNr = rawElement.position.start.line
            let line = lines[lineNr]
            
            if (!line) continue;

            // Esta linha estava com um bug sutil, corrigido para usar o método correto do deserializer
            const components = DESERIALIZERS[format].extractComponents(line)
            if (!components) {
                continue
            }
            let detail = DESERIALIZERS[format].deserialize(components.body)

            let [actual, expected] = detail.pomodoros.split('/')

            const dateformat = 'YYYY-MM-DD'
            let item: TaskItem = {
                text: line,
                path: file.path,
                fileName: file.name,
                name: detail.description,
                status: components.status,
                blockLink: components.blockLink,
                checked: rawElement.task != '' && rawElement.task != ' ',
                description: detail.description,
                done: detail.doneDate?.format(dateformat) ?? '',
                due: detail.dueDate?.format(dateformat) ?? '',
                created: detail.createdDate?.format(dateformat) ?? '',
                cancelled: detail.cancelledDate?.format(dateformat) ?? '',
                scheduled: detail.scheduledDate?.format(dateformat) ?? '',
                start: detail.startDate?.format(dateformat) ?? '',
                priority: detail.priority,
                expected: expected ? parseInt(expected) : 0,
                actual: actual === '' ? 0 : parseInt(actual),
                tags: detail.tags,
                line: lineNr,
            }

            cache[lineNr] = item
        }
    }

    return Object.values(cache)
}
...
# HUB - Temporada %%NUMERO_TEMPORADA%%: %%NOME_SERIE%%
...
actions:
  - type: inlineJS
    code: |
      ...
      const macroName = "Adicionar Episódios a temporada (Série)";
...
const pages = dv.pages(`"${pastaDoModulo}"`)
    ...
    .where(p => p.tipo === 'serie_episodio' && p.hub_uid === hubModulo.hub_uid)

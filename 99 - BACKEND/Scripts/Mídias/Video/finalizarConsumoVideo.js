// SOTA - Script para Finalizar o Consumo de um Vídeo v1.0
...
module.exports = async (params) => {
    ...
    class ConfirmarFinalizacaoModal extends Modal {
        ...
        onOpen() {
            this.contentEl.createEl("h2", { text: "Finalizar Consumo do Vídeo?" });
    ...
    try {
        ...
        if (cicloObj && cicloObj.status === 'concluido') {
            new Notice(`ℹ️ O Ciclo ${cicloAtualNum} deste vídeo já está finalizado.`);
            return;
        }
        ...
        new Notice("🚀 Finalizando ciclo de leitura do vídeo...");
        ...
        new Notice(`✅ Consumo do vídeo finalizado!`);

    } catch (e) {
        new Notice("❌ ERRO crítico ao finalizar consumo. Verifique o console.");
        console.error("Erro em finalizarConsumoVídeoAtomico.js:", e);
    }
};
// SOTA - Script para Iniciar Novo Ciclo de Consumo (Vídeo) v2.0
...
module.exports = async (params) => {
    ...
    class ConfirmarNovoCicloModal extends Modal {
        ...
        onOpen() {
            ...
            this.contentEl.createEl("p", { text: "Esta ação é para reativar o vídeo. O ciclo atual será arquivado e um novo será criado." });
    ...
    try {
        ...
        if (!(activeFile instanceof TFile)) { new Notice("❌ ERRO: Arquivo HUB do vídeo não encontrado."); return; }
        ...
        const pastaVídeoCompleta = activeFile.parent.parent.path; // Sobe do /HUB para a raiz do projeto do vídeo
        const sota_uid = fm.sota_uid;

        // Cria a pasta para o novo ciclo
        const pastaCicloNovo = `${pastaVídeoCompleta}/Anotações/Ciclo_${String(novoCicloNum).padStart(2, '0')}`;
        if (await app.vault.adapter.exists(pastaCicloNovo)) {
            ...
            await criarArquivo(
                ...
                "99 - BACKEND/Templates/Midias/Video/Video_Anotacoes_Template.md",
                { '%%SOTA_UID%%': sota_uid, '%%NUMERO_CICLO%%': novoCicloNum }
            );

            await criarArquivo(
                `${pastaCicloNovo}/${nomeExcalidraw}`,
                "99 - BACKEND/Templates/Midias/Video/Excalidraw_Template_Vazio_Video.md"
            );
        }
        ...
        new Notice(`✅ Novo Ciclo de Consumo (${novoCicloNum}) iniciado para o vídeo!`);

    } catch (e) {
        new Notice("❌ ERRO crítico ao iniciar novo ciclo. Verifique o console.");
        console.error("Erro em iniciarNovoCicloVídeoAtomico.js:", e);
    }
};
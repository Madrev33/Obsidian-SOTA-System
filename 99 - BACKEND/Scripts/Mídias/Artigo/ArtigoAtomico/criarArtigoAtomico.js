// SOTA - Script de Criação de Artigo Atômico v3.0 (Rich UI)
// Implementa estrutura de ciclos e artefatos por ciclo com interface gráfica.
...
module.exports = async (params) => {
    ...
    // --- 1. ESTILOS CSS ---
    const styleId = 'sota-create-media-styles';
    ...
    if (!existingStyle) {
        ...
        style.innerHTML = `
            ...
            /* CSS ROBUSTO (Estilo do CriarExercicio.js) */
            .sota-input, .sota-dropdown { 
                width: 100%; 
                background-color: var(--background-primary); 
                border: 1px solid var(--background-modifier-border); 
                border-radius: 6px; 
                padding: 5px 12px; /* Padding padrão do Obsidian */
    ...
    class CreateArticleModal extends Modal {
        ...
        onOpen() {
            ...
            contentEl.createDiv({ cls: "sota-header", text: "📄 Novo Artigo" });
            ...
            nameInput.setPlaceholder("Ex: O Futuro da IA").onChange(v => this.data.nome = v);
    ...
    // --- 3. LÓGICA DE CRIAÇÃO (Extraída e mantida intacta) ---
    async function processarCriacao(data) {
        ...
        new Notice("🚀 Criando artigo, aguarde...");

        const config = {
            template_hub: "99 - BACKEND/Templates/Midias/Artigo/ArtigoAtomico/Artigo_Atomico_Template.md",
            template_anotacoes_ciclo: "99 - BACKEND/Templates/Midias/Artigo/ArtigoAtomico/Artigo_Atomico_Anotacoes_Template.md",
            template_excalidraw: "99 - BACKEND/Templates/Midias/Artigo/ArtigoAtomico/Excalidraw_Template_Vazio_Artigo_Atomico.md",
        ...
        try {
            ...
            const pastaCiclosRaiz = `${pastaArtigoCompleta}/Anotações`;
            ...
            const id_midia = sanitizar(nomePastaArtigo);
            const pastaLogMetricas = `99 - BACKEND/Logs_Metricas/Artigos/${id_midia}`;
            ...
            const nomeHub = `00. HUB - ${nomePastaArtigo}.md`;
            const caminhoHub = `${pastaArtigoCompleta}/HUB/${nomeHub}`;
            const placeholdersHub = {
                '%%TITLE%%': nomeDoArtigo, '%%SOTA_UID%%': sota_uid, '%%ID_MIDIA%%': id_midia,
            ...
            new Notice(`✅ Projeto de Artigo "${nomeDoArtigo}" criado com sucesso!`);
            app.workspace.getLeaf(true).openFile(arquivoHub);
        
        } catch (error) {
            console.error("Erro ao criar Artigo Atômico:", error);
            new Notice(`❌ Ocorreu um erro. Verifique o console: ${error.message}`);
        }
    }

    // --- 4. EXECUÇÃO ---
    new CreateArticleModal(app).open();
};
// SOTA - Script para Iniciar Novo Ciclo de Leitura (Artigo Atômico) v2.0
...
module.exports = async (params) => {
    ...
    class ConfirmarNovoCicloModal extends Modal {
        ...
        onOpen() {
            ...
            this.contentEl.createEl("p", { text: "Esta ação é para reler o artigo. O ciclo atual será arquivado e um novo será criado." });
    ...
    try {
        ...
        const pastaArtigoCompleta = activeFile.parent.parent.path; // Sobe do /HUB para a raiz do projeto do artigo
        const sota_uid = fm.sota_uid;

        // Cria a pasta para o novo ciclo
        const pastaCicloNovo = `${pastaArtigoCompleta}/Anotações/Ciclo_${String(novoCicloNum).padStart(2, '0')}`;
        if (await app.vault.adapter.exists(pastaCicloNovo)) {
            ...
            await criarArquivo(
                ...
                "99 - BACKEND/Templates/Midias/Artigo/ArtigoAtomico/Artigo_Atomico_Anotacoes_Template.md",
                { '%%SOTA_UID%%': sota_uid, '%%NUMERO_CICLO%%': novoCicloNum }
            );

            await criarArquivo(
                `${pastaCicloNovo}/${nomeExcalidraw}`,
                "99 - BACKEND/Templates/Midias/Artigo/ArtigoAtomico/Excalidraw_Template_Vazio_Artigo_Atomico.md"
            );
        }
        ...
        new Notice(`✅ Novo Ciclo de Leitura (${novoCicloNum}) iniciado para o artigo!`);

    } catch (e) {
        new Notice("❌ ERRO crítico ao iniciar novo ciclo. Verifique o console.");
        console.error("Erro em iniciarNovoCicloArtigoAtomico.js:", e);
    }
};
// SOTA - Script para Adicionar Temporadas a uma Podcast
...
module.exports = async (params) => {
    ...
    if (!activeFilePath) {
        new Notice("❌ ERRO: Caminho do arquivo HUB da Podcast não recebido pelo script.");
        return;
    }
    
    const activeFile = app.vault.getAbstractFileByPath(activeFilePath);
    if (!(activeFile instanceof TFile)) {
        new Notice("❌ ERRO: Não foi possível encontrar o arquivo HUB da Podcast.");
        return;
    }
    ...
    if (hubCurso.status === 'concluido') {
        new Notice("⚠️ Para adicionar uma nova temporada, primeiro reative a podcast.");
        return; 
    }
    ...
    const templateModuloFile = app.vault.getAbstractFileByPath("99 - BACKEND/Templates/Midias/Podcast/PodcastSerializado/Temporada_Template.md");
    ...
    let conteudoFinalModulo = conteudoModuloBase
        ...
        .replace(/%%NOME_PODCAST%%/g, hubCurso.file.name.replace('00. HUB - ', ''));

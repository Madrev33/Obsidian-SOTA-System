// SOTA - iniciarNovoCicloSerie.js v4.0
...
module.exports = async (params) => {
    ...
    class ConfirmarArquivamentoModal extends Modal {
        ...
        onOpen() {
            ...
            this.contentEl.createEl("p", { text: "Uma nova estrutura de pastas será criada para o próximo ciclo, permitindo que vc assista todo o Podcast novamente do zero." });
            this.contentEl.createEl("strong", { text: "USE ESSA OPÇÃO APENAS QUANDO QUISER ASSISTIR TODO O PODCAST DO ZERO NOVAMENTE!" });
    ...
    try {
        ...
        if (!(hubFile instanceof TFile)) { new Notice("❌ ERRO: Não foi possível encontrar o HUB da Podcast."); return; }
        ...
        if (!await app.vault.adapter.exists(pastaCicloNovo)) {
            ...
            if (totalModulos > 0) {
                const templateModuloPath = "99 - BACKEND/Templates/Midias/Podcast/PodcastSerializado/Temporada_Template.md";
                ...
                for (let i = 1; i <= totalModulos; i++) {
                    ...
                    let conteudoFinalModulo = conteudoModuloBase
                        ...
                        .replace(/%%NOME_PODCAST%%/g, hubFile.basename.replace('00. HUB - ', ''));
        ...
        console.error("Erro no script iniciarNovoCicloPodcast:", error);

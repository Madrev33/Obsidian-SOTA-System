// SOTA - Script para Reativar a Podcast
...
module.exports = async (params) => {
    ...
    class ConfirmarReativacaoModal extends Modal {
        ...
        onOpen() {
            this.contentEl.createEl("h2", { text: "Reativar Podcast?" });
            this.contentEl.createEl("p", { text: "Esta ação marcará o Podcast como 'ativo' novamente, permitindo que você adicione novas temporadas." });
            this.contentEl.createEl("p", { text: "Use esta opção se novos episódios foram lançados ou se você decidiu revisitar partes do podcast sem iniciar um novo ciclo." });
    ...
    if (!activeFile) { new Notice("❌ ERRO: Não foi possível encontrar o arquivo HUB da Podcast."); return; }
    ...
    if (fm.status !== 'concluido') {
        new Notice("ℹ️ A Podcast já está ativa.");
        return; 
    }
    ...
    new Notice("🚀 Reativando o Podcast...");
    ...
    if (erro) {
      ...
      new Notice(`✅ Podcast reativada! Pronto para continuar assistindo no Ciclo ${cicloAfetado}.`);
    } else {

// SOTA - Script de Criação de Podcast Único v3.0 (Rich UI)
// Interface unificada para criar micro-projetos de podcast.
...
module.exports = async (params) => {
    ...
    // --- 1. ESTILOS CSS ---
    const styleId = 'sota-create-pod-styles';
    ...
    // --- 2. CLASSE DO MODAL ---
    class CreatePodcastModal extends Modal {
        constructor(app) {
            super(app);
            this.data = {
                nome: "",
        ...
        onOpen() {
            ...
            contentEl.createDiv({ cls: "sota-header", text: "🎙️ Novo Podcast" });

            // 1. Nome
            const wrapName = contentEl.createDiv({ cls: "sota-input-wrapper" });
            wrapName.createDiv({ cls: "sota-label", text: "Título do Podcast" });
            const nameInput = new TextComponent(wrapName);
            nameInput.inputEl.addClass("sota-input");
            nameInput.setPlaceholder("Ex: Tal Tal #105").onChange(v => this.data.nome = v);
    ...
    async function processarCriacao(data) {
        const nomeDoPodcast = data.nome;
        const soberaniaSelecionada = data.soberania;
        const escolhaAcao = data.acao;

        new Notice("🚀 Criando micro-projeto de podcast, aguarde...");

        const config = {
            template_hub: "99 - BACKEND/Templates/Midias/Podcast/PodcastAtomico/Podcast_Unico_Template.md",
            template_anotacoes_ciclo: "99 - BACKEND/Templates/Midias/Podcast/PodcastAtomico/Podcast_Unico_Anotacoes_Template.md",
            template_excalidraw: "99 - BACKEND/Templates/Midias/Podcast/PodcastAtomico/Excalidraw_Template_Vazio_Podcast_Unico.md",
        ...
        const nomePastaPodcast = nomeDoPodcast.replace(/[\\/:"*?<>|#^\[\]]+/g, '').trim();
        const pastaDestinoRaiz = escolhaAcao === 'ativo' ? config.pastaProgresso : config.pastaConsumir;
        const pastaPodcastCompleta = `${pastaDestinoRaiz}/${nomePastaPodcast}`;

        if (await app.vault.adapter.exists(pastaPodcastCompleta)) {
            new Notice(`❌ ERRO: Uma pasta para "${nomePastaPodcast}" já existe.`);
            return;
        }

        try {
            // --- CRIAÇÃO DA ESTRUTURA DE PASTAS ---
            await app.vault.createFolder(pastaPodcastCompleta);
            await app.vault.createFolder(`${pastaPodcastCompleta}/HUB`);
            const pastaCiclosRaiz = `${pastaPodcastCompleta}/Anotações`;
            ...
            const id_midia = sanitizar(nomePastaPodcast);
            const pastaLogMetricas = `99 - BACKEND/Logs_Metricas/Podcasts/${id_midia}`;
            ...
            const nomeHub = `00. HUB - ${nomePastaPodcast}.md`;
            const caminhoHub = `${pastaPodcastCompleta}/HUB/${nomeHub}`;
            const placeholdersHub = {
                '%%TITLE%%': nomeDoPodcast, '%%SOTA_UID%%': sota_uid, '%%ID_MIDIA%%': id_midia,
            ...
            new Notice(`✅ Projeto de Podcast "${nomeDoPodcast}" criado com sucesso!`);
            app.workspace.getLeaf(true).openFile(arquivoHub);
        
        } catch (error) {
            console.error("Erro ao criar Podcast Único:", error);
            new Notice(`❌ Ocorreu um erro. Verifique o console: ${error.message}`);
        }
    }

    // --- 4. EXECUÇÃO ---
    new CreatePodcastModal(app).open();
};
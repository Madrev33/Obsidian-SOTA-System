// SOTA - Script para Finalizar o Consumo de um Filme v1.0
...
module.exports = async (params) => {
    ...
    class ConfirmarFinalizacaoModal extends Modal {
        ...
        onOpen() {
            this.contentEl.createEl("h2", { text: "Finalizar Consumo do Filme?" });
            this.contentEl.createEl("p", { text: "Esta ação marcará o ciclo atual como 'concluído' e registrará a data de finalização." });
    ...
    try {
        ...
        if (cicloObj && cicloObj.status === 'concluido') {
            new Notice(`ℹ️ O Ciclo ${cicloAtualNum} deste filme já está finalizado.`);
            return;
        }
        ...
        new Notice("🚀 Finalizando ciclo de leitura do filme...");
        ...
        await app.fileManager.processFrontMatter(activeFile, (fm) => {
            ...
            fm.consumo_status = "concluido";
        });

        new Notice(`✅ Consumo do filme finalizado!`);

    } catch (e) {
        new Notice("❌ ERRO crítico ao finalizar consumo. Verifique o console.");
        console.error("Erro em finalizarConsumoFilmeAtomico.js:", e);
    }
};
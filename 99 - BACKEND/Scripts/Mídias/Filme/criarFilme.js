// SOTA - Script de Criação de Filme v3.0 (Rich UI)
// Interface unificada para criar micro-projetos de filme.
...
module.exports = async (params) => {
    ...
    // --- 1. ESTILOS CSS ---
    const styleId = 'sota-create-film-styles';
    ...
    // --- 2. CLASSE DO MODAL ---
    class CreateFilmModal extends Modal {
        constructor(app) {
            super(app);
            this.data = {
                nome: "",
        ...
        onOpen() {
            ...
            contentEl.createDiv({ cls: "sota-header", text: "🎬 Novo Filme" });

            // 1. Nome
            const wrapName = contentEl.createDiv({ cls: "sota-input-wrapper" });
            wrapName.createDiv({ cls: "sota-label", text: "Título do Filme" });
            const nameInput = new TextComponent(wrapName);
            nameInput.inputEl.addClass("sota-input");
            nameInput.setPlaceholder("Ex: Interstellar").onChange(v => this.data.nome = v);
    ...
    async function processarCriacao(data) {
        const nomeDoFilme = data.nome;
        const soberaniaSelecionada = data.soberania;
        const escolhaAcao = data.acao;

        new Notice("🚀 Criando micro-projeto de filme, aguarde...");

        const config = {
            template_hub: "99 - BACKEND/Templates/Midias/Filme/Filme_Template.md",
            template_anotacoes_ciclo: "99 - BACKEND/Templates/Midias/Filme/Filme_Anotacoes_Template.md",
            template_excalidraw: "99 - BACKEND/Templates/Midias/Filme/Excalidraw_Template_Vazio_Filme.md",
            pastaConsumir: "03 - Conhecimento/01 - Mídias/01. Para Consumir/Filmes",
            pastaProgresso: "03 - Conhecimento/01 - Mídias/02. Em Progresso/Filmes"
        };
        
        const nomePastaFilme = nomeDoFilme.replace(/[\\/:"*?<>|#^\[\]]+/g, '').trim();
        const pastaDestinoRaiz = escolhaAcao === 'ativo' ? config.pastaProgresso : config.pastaConsumir;
        const pastaFilmeCompleta = `${pastaDestinoRaiz}/${nomePastaFilme}`;

        if (await app.vault.adapter.exists(pastaFilmeCompleta)) {
            new Notice(`❌ ERRO: Uma pasta para "${nomePastaFilme}" já existe.`);
            return;
        }

        try {
            // --- CRIAÇÃO DA ESTRUTURA DE PASTAS ---
            await app.vault.createFolder(pastaFilmeCompleta);
            await app.vault.createFolder(`${pastaFilmeCompleta}/HUB`);
            const pastaCiclosRaiz = `${pastaFilmeCompleta}/Anotações`;
            ...
            const id_midia = sanitizar(nomePastaFilme);
            const pastaLogMetricas = `99 - BACKEND/Logs_Metricas/Filmes/${id_midia}`;
            ...
            const nomeHub = `00. HUB - ${nomePastaFilme}.md`;
            const caminhoHub = `${pastaFilmeCompleta}/HUB/${nomeHub}`;
            const placeholdersHub = {
                '%%TITLE%%': nomeDoFilme, '%%SOTA_UID%%': sota_uid, '%%ID_MIDIA%%': id_midia,
            ...
            new Notice(`✅ Projeto de Filme "${nomeDoFilme}" criado com sucesso!`);

// SOTA - Script para Iniciar Novo Ciclo de Consumo (Filme) v2.0
...
module.exports = async (params) => {
    ...
    class ConfirmarNovoCicloModal extends Modal {
        ...
        onOpen() {
            ...
            this.contentEl.createEl("p", { text: "Esta ação é para reativar o filme. O ciclo atual será arquivado e um novo será criado." });
    ...
    try {
        ...
        if (!(activeFile instanceof TFile)) { new Notice("❌ ERRO: Arquivo HUB do filme não encontrado."); return; }
        ...
        new Notice("🚀 Iniciando novo ciclo do Video...");

        // --- ETAPA 3: AÇÃO ---
        const novoCicloNum = cicloAtualNum + 1;
        const pastaFilmeCompleta = activeFile.parent.parent.path; // Sobe do /HUB para a raiz do projeto do filme
        const sota_uid = fm.sota_uid;

        // Cria a pasta para o novo ciclo
        const pastaCicloNovo = `${pastaFilmeCompleta}/Anotações/Ciclo_${String(novoCicloNum).padStart(2, '0')}`;
        if (await app.vault.adapter.exists(pastaCicloNovo)) {
            ...
            await criarArquivo(
                ...
                "99 - BACKEND/Templates/Midias/Filme/Filme_Anotacoes_Template.md",
                { '%%SOTA_UID%%': sota_uid, '%%NUMERO_CICLO%%': novoCicloNum }
            );

            await criarArquivo(
                `${pastaCicloNovo}/${nomeExcalidraw}`,
                "99 - BACKEND/Templates/Midias/Filme/Excalidraw_Template_Vazio_Filme.md"
            );
        }
        ...
        new Notice(`✅ Novo Ciclo de Consumo (${novoCicloNum}) iniciado para o filme!`);

    } catch (e) {
        new Notice("❌ ERRO crítico ao iniciar novo ciclo. Verifique o console.");
        console.error("Erro em iniciarNovoCicloFilmeAtomico.js:", e);
    }
};
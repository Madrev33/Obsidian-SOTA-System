// SOTA - Script de Criação de Documentação v3.0 (Rich UI)
// Interface gráfica unificada para criar documentações com estrutura de seções.
...
module.exports = async (params) => {
    ...
    // --- 1. ESTILOS CSS ---
    const styleId = 'sota-create-doc-styles';
    ...
    // --- 2. CLASSE DO MODAL ---
    class CreateDocModal extends Modal {
        constructor(app) {
            super(app);
            this.data = {
                nome: "",
        ...
        onOpen() {
            ...
            contentEl.createDiv({ cls: "sota-header", text: "📑 Nova Documentação" });

            // 1. Nome
            const wrapName = contentEl.createDiv({ cls: "sota-input-wrapper" });
            wrapName.createDiv({ cls: "sota-label", text: "Nome da Documentação" });
            const nameInput = new TextComponent(wrapName);
            nameInput.inputEl.addClass("sota-input");
            nameInput.setPlaceholder("Ex: Documentação da API").onChange(v => this.data.nome = v);
            setTimeout(() => nameInput.inputEl.focus(), 50);

            // 2. Seções
            const wrapSecoes = contentEl.createDiv({ cls: "sota-input-wrapper" });
            ...
            secoesInput.setValue("1").onChange(v => this.data.secoes = parseInt(v) || 1);
    ...
    async function processarCriacao(data) {
        ...
        const config = {
            template_hub: "99 - BACKEND/Templates/Midias/Documentacao/Documentacao_Paginado_HUB_Template.md",
            template_secao: "99 - BACKEND/Templates/Midias/Documentacao/Documentacao_Paginado_Secao_Template.md", // Caminho unificado
            pastaConsumir: "03 - Conhecimento/01 - Mídias/01. Para Consumir/Documentações",
            pastaProgresso: "03 - Conhecimento/01 - Mídias/02. Em Progresso/Documentações"
        };
        ...
        try {
            ...
            const pastaLogMetricas = `99 - BACKEND/Logs_Metricas/Documentacoes/${nomeArtigoSanitizado}`;
            ...
            if (totalSeções > 0) {
                ...
                for (let i = 1; i <= totalSeções; i++) {
                    ...
                    try {
                        const templateExcalidrawPath = "99 - BACKEND/Templates/Midias/Documentacao/Excalidraw_Template_Vazio_Documentacao_Paginado.md";
            ...
            new Notice(`✅ Projeto de Documentação "${nomeDoArtigo}" criado com sucesso!`);
            app.workspace.getLeaf(true).openFile(arquivoHub);
        } catch (error) {
            console.error("Erro ao criar Projeto de Documentação:", error);
            new Notice(`❌ Ocorreu um erro. Verifique o console: ${error.message}`);
        }
    }

    // --- 4. EXECUÇÃO ---
    new CreateDocModal(app).open();
};
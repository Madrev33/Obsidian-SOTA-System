// SOTA - Script para Finalizar o Consumo de um Jogo (Pausa Longa / Zerado) v1.0
// v1.0 - Adiciona modal de confirmação antes de executar a ação.
...
module.exports = async (params) => {
    ...
    class ConfirmarFinalizacaoModal extends Modal {
        ...
        onOpen() {
            this.contentEl.createEl("h2", { text: "Finalizar Jogo (Zerado)?" });
            this.contentEl.createEl("p", { text: "Esta ação marca o Jogo como 'concluído', indicando que você terminou (zerou) o conteúdo disponível no momento." });

            new Setting(this.contentEl)
                .addButton(btn => btn
                    .setButtonText("Confirmar e Finalizar")
                    .setCta()
                    .onClick(() => {
    ...
    if (!activeFile) {
        new Notice("❌ ERRO: Não foi possível encontrar o arquivo HUB do Jogo.");
        return;
    }
    ...
    new Notice("🚀 Registrando finalização do jogo...");
    ...
    try {
      await app.fileManager.processFrontMatter(activeFile, (fm) => {
          ...
          if (cicloObj.status === 'concluido' && fm.status === 'concluido') {
              erro = `O ciclo ${cicloAtualNum} já está marcado como finalizado.`;
              return;
          }
          ...
          // Limpa campos legados se existirem
    ...
    if (erro) {
      ...
      new Notice(`✅ Jogo Zerado (Ciclo ${cicloAfetado} finalizado)!`);
    } else {

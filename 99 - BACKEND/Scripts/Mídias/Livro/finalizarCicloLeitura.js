// SOTA - Script para Finalizar um Ciclo de Leitura de Livro
// v5.0 - Padrão de Validação Robusta e Modal Assíncrono Encapsulado
...
module.exports = async (params) => {
    ...
    // --- CLASSE DO MODAL REATORADA (PADRÃO SOTA DE COMPONENTE ASSÍNCRONO) ---
    class ConfirmarFinalizacaoModal extends Modal {
        ...
        // Este método transforma o modal em um objeto "awaitable"
        waitForDecision() {
            return this.promise;
        }

        onOpen() {
            this.contentEl.createEl("h2", { text: "Finalizar Ciclo de Leitura?" });
    ...
    if (!(activeFile instanceof TFile)) { new Notice("❌ ERRO: Não foi possível encontrar o arquivo HUB do Livro."); return; }
    ...
    try {
        ...
        if (cicloObj && cicloObj.status === 'concluido') {
            new Notice(`⚠️ O Ciclo ${cicloAtualNum} já está finalizado.`);
            return; // ABORTA A EXECUÇÃO.
        }

    } catch (e) {
        new Notice("❌ ERRO crítico ao ler e validar o arquivo HUB.");
        console.error("Erro de validação em finalizarCicloLeitura.js:", e);
        return;
    }
    // --- FIM DA VALIDAÇÃO ---
    ...
    new Notice("🚀 Finalizando ciclo de leitura...");
    ...
    try {
      await app.fileManager.processFrontMatter(activeFile, (fm) => {
          ...
          // Dupla verificação para segurança máxima

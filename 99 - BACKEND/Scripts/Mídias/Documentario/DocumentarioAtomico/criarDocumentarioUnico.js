// SOTA - Script de Criação de Documentário Único v3.0 (Rich UI)
// Interface gráfica unificada para criar micro-projetos de documentário.
...
module.exports = async (params) => {
    ...
    // --- 1. ESTILOS CSS ---
    const styleId = 'sota-create-docu-styles';
    ...
    // --- 2. CLASSE DO MODAL ---
    class CreateDocuModal extends Modal {
        constructor(app) {
            super(app);
            this.data = {
                nome: "",
        ...
        onOpen() {
            ...
            contentEl.createDiv({ cls: "sota-header", text: "📽️ Novo Documentário" });

            // 1. Nome
            const wrapName = contentEl.createDiv({ cls: "sota-input-wrapper" });
            wrapName.createDiv({ cls: "sota-label", text: "Título do Documentário" });
            const nameInput = new TextComponent(wrapName);
            nameInput.inputEl.addClass("sota-input");
            nameInput.setPlaceholder("Ex: The Social Dilemma").onChange(v => this.data.nome = v);
    ...
    async function processarCriacao(data) {
        const nomeDoDocumentário = data.nome;
        const soberaniaSelecionada = data.soberania;
        const escolhaAcao = data.acao;

        new Notice("🚀 Criando micro-projeto de documentário, aguarde...");

        const config = {
            template_hub: "99 - BACKEND/Templates/Midias/Documentario/DocumentarioAtomico/Documentario_Unico_Template.md",
            template_anotacoes_ciclo: "99 - BACKEND/Templates/Midias/Documentario/DocumentarioAtomico/Documentario_Unico_Anotacoes_Template.md",
            template_excalidraw: "99 - BACKEND/Templates/Midias/Documentario/DocumentarioAtomico/Excalidraw_Template_Vazio_Documentario_Unico.md",
            pastaConsumir: "03 - Conhecimento/01 - Mídias/01. Para Consumir/Documentários",
            pastaProgresso: "03 - Conhecimento/01 - Mídias/02. Em Progresso/Documentários"
        };
        
        const nomePastaDocumentário = nomeDoDocumentário.replace(/[\\/:"*?<>|#^\[\]]+/g, '').trim();
        const pastaDestinoRaiz = escolhaAcao === 'ativo' ? config.pastaProgresso : config.pastaConsumir;
        const pastaDocumentárioCompleta = `${pastaDestinoRaiz}/${nomePastaDocumentário}`;

        if (await app.vault.adapter.exists(pastaDocumentárioCompleta)) {
            new Notice(`❌ ERRO: Uma pasta para "${nomePastaDocumentário}" já existe.`);
            return;
        }

        try {
            // --- CRIAÇÃO DA ESTRUTURA DE PASTAS ---
            await app.vault.createFolder(pastaDocumentárioCompleta);
            await app.vault.createFolder(`${pastaDocumentárioCompleta}/HUB`);
            const pastaCiclosRaiz = `${pastaDocumentárioCompleta}/Anotações`;
            ...
            const id_midia = sanitizar(nomePastaDocumentário);
            const pastaLogMetricas = `99 - BACKEND/Logs_Metricas/Documentarios/${id_midia}`;
            ...
            const nomeHub = `00. HUB - ${nomePastaDocumentário}.md`;
            const caminhoHub = `${pastaDocumentárioCompleta}/HUB/${nomeHub}`;
            const placeholdersHub = {
                '%%TITLE%%': nomeDoDocumentário, '%%SOTA_UID%%': sota_uid, '%%ID_MIDIA%%': id_midia,
            ...
            new Notice(`✅ Projeto de Documentário "${nomeDoDocumentário}" criado com sucesso!`);
            app.workspace.getLeaf(true).openFile(arquivoHub);
        
        } catch (error) {
            console.error("Erro ao criar Documentário Único:", error);
            new Notice(`❌ Ocorreu um erro. Verifique o console: ${error.message}`);
        }
    }

    // --- 4. EXECUÇÃO ---
    new CreateDocuModal(app).open();
};
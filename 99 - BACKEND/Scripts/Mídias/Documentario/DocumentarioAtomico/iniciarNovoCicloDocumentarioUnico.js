// SOTA - Script para Iniciar Novo Ciclo de Consumo (Documentário Único) v2.0
...
module.exports = async (params) => {
    ...
    class ConfirmarNovoCicloModal extends Modal {
        ...
        onOpen() {
            ...
            this.contentEl.createEl("p", { text: "Esta ação é para assistir novamente o documentário. O ciclo atual será arquivado e um novo será criado." });
    ...
    try {
        ...
        const pastaDocumentárioCompleta = activeFile.parent.parent.path; // Sobe do /HUB para a raiz do projeto do documentário
        const sota_uid = fm.sota_uid;

        // Cria a pasta para o novo ciclo
        const pastaCicloNovo = `${pastaDocumentárioCompleta}/Anotações/Ciclo_${String(novoCicloNum).padStart(2, '0')}`;
        if (await app.vault.adapter.exists(pastaCicloNovo)) {
            ...
            await criarArquivo(
                ...
                "99 - BACKEND/Templates/Midias/Documentario/DocumentarioAtomico/Documentario_Unico_Anotacoes_Template.md",
                { '%%SOTA_UID%%': sota_uid, '%%NUMERO_CICLO%%': novoCicloNum }
            );

            await criarArquivo(
                `${pastaCicloNovo}/${nomeExcalidraw}`,
                "99 - BACKEND/Templates/Midias/Documentario/DocumentarioAtomico/Excalidraw_Template_Vazio_Documentario_Unico.md"
            );
        }
        ...
        new Notice(`✅ Novo Ciclo de Consumo (${novoCicloNum}) iniciado para o documentário!`);

    } catch (e) {
        new Notice("❌ ERRO crítico ao iniciar novo ciclo. Verifique o console.");
        console.error("Erro em iniciarNovoCicloDocumentárioAtomico.js:", e);
    }
};
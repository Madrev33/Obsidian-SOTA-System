// SOTA - Script de Criação de Documentários Seriados v2.0 (Rich UI)
// Interface unificada para criar documentários com estrutura de temporadas.
...
module.exports = async (params) => {
    ...
    // --- 1. ESTILOS CSS ---
    const styleId = 'sota-create-series-styles';
    ...
    // --- 2. CLASSE DO MODAL ---
    class CreateDocSeriesModal extends Modal {
        constructor(app) {
            super(app);
            this.data = {
                nome: "",
        ...
        onOpen() {
            ...
            contentEl.createDiv({ cls: "sota-header", text: "📽️ Novo Doc. Seriado" });

            // 1. Nome
            const wrapName = contentEl.createDiv({ cls: "sota-input-wrapper" });
            wrapName.createDiv({ cls: "sota-label", text: "Nome do Documentário" });
            const nameInput = new TextComponent(wrapName);
            nameInput.inputEl.addClass("sota-input");
            nameInput.setPlaceholder("Ex: Planet Earth").onChange(v => this.data.nome = v);
    ...
    async function processarCriacao(data) {
        ...
        new Notice("🚀 Criando ecossistema do Documentário...");

        const config = {
            template_hub: "99 - BACKEND/Templates/Midias/Documentario/DocumentarioSerializado/DocumentarioSerializado_Template.md",
            template_modulo: "99 - BACKEND/Templates/Midias/Documentario/DocumentarioSerializado/Temporada_Template.md",
            template_aula: "99 - BACKEND/Templates/Midias/Documentario/DocumentarioSerializado/Episodio_Template.md",
        ...
        if (await app.vault.adapter.exists(pastaCursoCompleta)) {
            new Notice(`❌ ERRO: Uma pasta para "${nomePastaCurso}" já existe.`);
            return;
        }
        ...
        try {
            ...
            const pastaLogMetricas = `99 - BACKEND/Logs_Metricas/Documentarios/${nomeCursoSanitizado}`;
            ...
            if (totalModulos > 0) {
                for (let i = 1; i <= totalModulos; i++) {
                    ...
                    await criarArquivo(caminhoHubModulo, config.template_modulo, {
                        ...
                        '%%NOME_DOCUMENTARIO%%': nomeDoCurso,
                    });
                }
            }

            new Notice(`✅ Documentário "${nomeDoCurso}" criado com sucesso!`);
            app.workspace.getLeaf(true).openFile(arquivoHub);
        
        } catch (error) {
            console.error("Erro ao criar Documentário:", error);
            new Notice(`❌ Ocorreu um erro. Verifique o console: ${error.message}`);
        }
    }

    // --- 4. EXECUÇÃO ---
    new CreateDocSeriesModal(app).open();
};
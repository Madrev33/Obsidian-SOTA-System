// SOTA - Script para Adicionar Temporadas a uma Documentário
...
module.exports = async (params) => {
    ...
    if (!activeFilePath) {
        new Notice("❌ ERRO: Caminho do arquivo HUB da Documentário não recebido pelo script.");
        return;
    }
    
    const activeFile = app.vault.getAbstractFileByPath(activeFilePath);
    if (!(activeFile instanceof TFile)) {
        new Notice("❌ ERRO: Não foi possível encontrar o arquivo HUB da Documentário.");
        return;
    }
    ...
    if (hubCurso.status === 'concluido') {
        new Notice("⚠️ Para adicionar uma nova temporada, primeiro reative o documentário.");
        return; 
    }

    new Notice("🚀 Adicionando nova temporada...");
    ...
    const pastaCiclo = `${pastaCurso.path}/Temporadas/${cicloFormatado}`;
    ...
    const templateModuloFile = app.vault.getAbstractFileByPath("99 - BACKEND/Templates/Midias/Documentario/DocumentarioSerializado/Temporada_Template.md");
    if (!(templateModuloFile instanceof TFile)) {
        new Notice("❌ ERRO: Template de Temporada não encontrado."); return;
    }
    const conteudoModuloBase = await app.vault.read(templateModuloFile);

    const numeroPasta = novoNumeroModulo.toString().padStart(2, '0');
    const nomePastaModulo = `Temporada ${numeroPasta}`;
    const caminhoPastaModulo = `${pastaCiclo}/${nomePastaModulo}`;
    await app.vault.createFolder(caminhoPastaModulo);

    const nomeHubModulo = `00 - HUB Temporada ${numeroPasta}.md`;
    ...
    let conteudoFinalModulo = conteudoModuloBase
        ...
        .replace(/%%NUMERO_TEMPORADA%%/g, novoNumeroModulo.toString())
        .replace(/%%NOME_DOCUMENTARIO%%/g, hubCurso.file.name.replace('00. HUB - ', ''));
    await app.vault.create(caminhoHubModulo, conteudoFinalModulo);
    
    // --- 3. ATUALIZAÇÃO FINAL ---
    await app.fileManager.processFrontMatter(activeFile, (fm) => {
        fm.total_temporadas = novoNumeroModulo;
    });

    new Notice(`✅ Temporada ${novoNumeroModulo} adicionada com sucesso!`);
};
// SOTA - iniciarNovoCicloCurso.js v4.0
...
module.exports = async (params) => {
    ...
    class ConfirmarArquivamentoModal extends Modal {
        ...
        onOpen() {
            ...
            this.contentEl.createEl("p", { text: "Uma nova estrutura de pastas será criada para o próximo ciclo, permitindo que vc assista todo o curso do zero novamente." });
            this.contentEl.createEl("strong", { text: "USE ESSA OPÇÃO APENAS QUANDO QUISER ASSISTIR TODO O CURSO DO ZERO NOVAMENTE!" });
    ...
    try {
        ...
        if (!(hubFile instanceof TFile)) { new Notice("❌ ERRO: Não foi possível encontrar o HUB do Curso."); return; }
        ...
        const totalModulos = fm.total_modulos || 0;
        ...
        const pastaCicloNovo = `${pastaCursoCompleta}/Módulos/Ciclo_${String(novoCicloNum).padStart(2, '0')}`;
        if (!await app.vault.adapter.exists(pastaCicloNovo)) {
            await app.vault.createFolder(pastaCicloNovo);
            if (totalModulos > 0) {
                const templateModuloPath = "99 - BACKEND/Templates/Midias/Curso/Modulo_Template.md";
                const templateModuloFile = app.vault.getAbstractFileByPath(templateModuloPath);
                if (!(templateModuloFile instanceof TFile)) throw new Error(`Template de Módulo não encontrado.`);
                ...
                for (let i = 1; i <= totalModulos; i++) {
                    ...
                    const nomeHubModulo = `00 - HUB Módulo ${numeroModulo}.md`;
                    const caminhoHubModulo = `${pastaModuloPath}/${nomeHubModulo}`;
                    let conteudoFinalModulo = conteudoModuloBase
                        .replace(/%%HUB_UID%%/g, sota_uid)
                        .replace(/%%NUMERO_MODULO%%/g, i.toString())
                        .replace(/%%NOME_CURSO%%/g, hubFile.basename.replace('00. HUB - ', ''));
        ...
        await app.fileManager.processFrontMatter(hubFile, (fm) => {
            ...
            fm.modulo_atual = 1; fm.aula_atual = 1; fm.status = "parado";
            fm.sessao_ativa_timestamp_inicio = ""; fm.aulas_assistidas = 0;
            fm.ultima_aula_assistida = 0; fm.tempo_acumulado_aula_atual_segundos = 0;
        });

        new Notice(`✅ Novo Ciclo de Aprendizado (${novoCicloNum}) iniciado!`);

    } catch (error) {
        console.error("Erro no script iniciarNovoCicloCurso:", error);

// SOTA - Script de Criação de Séries v2.0 (Rich UI)
// Interface unificada para criar séries com estrutura de temporadas.
...
module.exports = async (params) => {
    ...
    // --- 1. ESTILOS CSS ---
    const styleId = 'sota-create-tvseries-styles';
    ...
    class CreateSeriesModal extends Modal {
        ...
        onOpen() {
            ...
            contentEl.createDiv({ cls: "sota-header", text: "📺 Nova Série" });

            // 1. Nome
            const wrapName = contentEl.createDiv({ cls: "sota-input-wrapper" });
            wrapName.createDiv({ cls: "sota-label", text: "Nome da Série" });
            const nameInput = new TextComponent(wrapName);
            nameInput.inputEl.addClass("sota-input");
            nameInput.setPlaceholder("Ex: Breaking Bad").onChange(v => this.data.nome = v);
    ...
    async function processarCriacao(data) {
        ...
        new Notice("🚀 Criando ecossistema da Série...");

        const config = {
            template_hub: "99 - BACKEND/Templates/Midias/Serie/Serie_Template.md",
            template_modulo: "99 - BACKEND/Templates/Midias/Serie/Temporada_Template.md",
            template_aula: "99 - BACKEND/Templates/Midias/Serie/Episodio_Template.md",
            pastaConsumir: "03 - Conhecimento/01 - Mídias/01. Para Consumir/Séries",
            pastaProgresso: "03 - Conhecimento/01 - Mídias/02. Em Progresso/Séries"
        };
        ...
        if (await app.vault.adapter.exists(pastaCursoCompleta)) {
            new Notice(`❌ ERRO: Uma pasta para a Série "${nomePastaCurso}" já existe.`);
            return;
        }
        ...
        try {
            ...
            const pastaLogMetricas = `99 - BACKEND/Logs_Metricas/Series/${nomeCursoSanitizado}`;
            ...
            if (totalModulos > 0) {
                for (let i = 1; i <= totalModulos; i++) {
                    ...
                    await criarArquivo(caminhoHubModulo, config.template_modulo, {
                        ...
                        '%%NOME_SERIE%%': nomeDoCurso,
                    });
                }
            }

            new Notice(`✅ Série "${nomeDoCurso}" criada com sucesso!`);
    ...
    // --- 4. EXECUÇÃO ---
    new CreateSeriesModal(app).open();
};
// SOTA - Script para Reativar a Série
...
module.exports = async (params) => {
    ...
    class ConfirmarReativacaoModal extends Modal {
        ...
        onOpen() {
            this.contentEl.createEl("h2", { text: "Reativar Série?" });
            this.contentEl.createEl("p", { text: "Esta ação marcará a Série como 'ativo' novamente, permitindo que você adicione novas temporadas." });
            this.contentEl.createEl("p", { text: "Use esta opção se novos episódios foram lançados ou se você decidiu revisitar partes da série sem iniciar um novo ciclo." });
    ...
    if (!activeFile) { new Notice("❌ ERRO: Não foi possível encontrar o arquivo HUB da Série."); return; }
    ...
    if (fm.status !== 'concluido') {
        new Notice("ℹ️ A Série já está ativa.");
        return; 
    }
    ...
    if (erro) {
      ...
      new Notice(`✅ Série reativada! Pronto para continuar assistindo no Ciclo ${cicloAfetado}.`);
    } else {

// SOTA - Script para Adicionar Episódios a uma Série
...
module.exports = async (params) => {
    ...
    const templateAulaFile = app.vault.getAbstractFileByPath("99 - BACKEND/Templates/Midias/Serie/Episodio_Template.md");
    ...
    const templateExcalidrawPath = "99 - BACKEND/Templates/Midias/Serie/Excalidraw_Template_Vazio_Serie.md"; // Ajuste o nome se necessário
    ...
    for (const child of pastaModulo.children) {
        if (child instanceof obsidian.TFolder && child.name.match(/^\d{2,} - /)) { // Verifica se é uma pasta de aula (nome começa com número)
            for (const subChild of child.children) {
                if (subChild instanceof TFile && dv.page(subChild.path)?.tipo === 'serie_episodio') {
                    contadorAulas++;

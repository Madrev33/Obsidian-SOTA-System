...
module.exports = async (params) => {
    ...
    class ConfirmarArquivamentoModal extends Modal {
        ...
        onOpen() {
            ...
            this.contentEl.createEl("p", { text: "Uma nova estrutura de pastas será criada para o próximo ciclo, permitindo que vc assista toda a Série novamente do zero." });
            this.contentEl.createEl("strong", { text: "USE ESSA OPÇÃO APENAS QUANDO QUISER ASSISTIR TODA A SÉRIE DO ZERO NOVAMENTE!" });
    ...
    try {
        ...
        if (!(hubFile instanceof TFile)) { new Notice("❌ ERRO: Não foi possível encontrar o HUB da Série."); return; }
        ...
        if (!await app.vault.adapter.exists(pastaCicloNovo)) {
            ...
            if (totalModulos > 0) {
                const templateModuloPath = "99 - BACKEND/Templates/Midias/Serie/Temporada_Template.md";
                ...
                for (let i = 1; i <= totalModulos; i++) {
                    ...
                    let conteudoFinalModulo = conteudoModuloBase
                        ...
                        .replace(/%%NOME_SERIE%%/g, hubFile.basename.replace('00. HUB - ', ''));
        ...
        console.error("Erro no script iniciarNovoCicloSerie:", error);

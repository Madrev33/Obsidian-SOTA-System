// SOTA - Script para Adicionar Temporadas a uma Série
...
module.exports = async (params) => {
    ...
    if (!activeFilePath) {
        new Notice("❌ ERRO: Caminho do arquivo HUB da Série não recebido pelo script.");
        return;
    }
    ...
    if (hubCurso.status === 'concluido') {
        new Notice("⚠️ Para adicionar uma nova temporada, primeiro reative a série.");
        return; 
    }
    ...
    const templateModuloFile = app.vault.getAbstractFileByPath("99 - BACKEND/Templates/Midias/Serie/Temporada_Template.md");
    ...
    let conteudoFinalModulo = conteudoModuloBase
        ...
        .replace(/%%NOME_SERIE%%/g, hubCurso.file.name.replace('00. HUB - ', ''));

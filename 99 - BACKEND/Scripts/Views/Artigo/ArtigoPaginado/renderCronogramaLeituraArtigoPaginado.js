// Componente SOTA: renderCronogramaLeituraArtigoPaginado.js v1.0
// Exibe o cronograma de leitura para Artigos Paginados, baseado no ciclo selecionado.
...
async function main() {
    ...
    const hubArray = dv.pages().where(p => p.id_midia === idMidia && p.tipo === "artigo_paginado_hub");
    if (hubArray.length === 0) {
        dv.paragraph(`❌ ERRO: HUB de Artigo Paginado com ID ${idMidia} não encontrado.`);
        return;
    }

// Componente SOTA: renderDiagnosticoInsightsDocumentacaoPaginado.js v1.1
// Exibe a contagem de insights por seção para Documentações Paginadas.
...
async function main() {
    ...
    const secoesFolder = `${documentacaoFolder}/Seções`;

    let secoes;
    // --- CORREÇÃO DE LÓGICA APLICADA AQUI ---
    if (cicloSelecionado === "Visão Agregada (Todos os Ciclos)") {
        secoes = dv.pages(`"${secoesFolder}"`).where(p => p.tipo === 'documentacao_paginado_secao' && p.hub_uid === hub.sota_uid);
    } else {
        ...
        secoes = dv.pages(`"${cicloFolderPath}"`).where(p => p.tipo === 'documentacao_paginado_secao' && p.hub_uid === hub.sota_uid);
    }

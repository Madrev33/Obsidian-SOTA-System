// Componente SOTA: renderKPIsEficienciaFilmeAtomico.js v2.0
...
async function main() {
    ...
    if (!hub) { dv.paragraph(`❌ ERRO: HUB de Filme com ID ${idMidia} não encontrado.`); return; }
    
    if (!cicloSelecionado) { dv.paragraph("⚠️ Selecione um ciclo para ver os KPIs."); return; }

    const idFilme = hub.id_midia;
    const logBrutoPath = `99 - BACKEND/Logs_Metricas/Filmes/${idFilme}/raw_logs.md`;

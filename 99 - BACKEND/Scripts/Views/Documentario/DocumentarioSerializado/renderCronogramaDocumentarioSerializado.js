// Componente SOTA: renderCronogramadocumentario.js 
async function main() {
    ...
    const hubArray = dv.pages().where(p => p.id_midia === idMidia && p.tipo === "documentario_serializado_hub");
    if (hubArray.length === 0) {
        dv.paragraph(`❌ ERRO: HUB de Documentário Serializado com ID ${idMidia} não encontrado.`);
        return;
    }

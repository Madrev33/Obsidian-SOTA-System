// Componente SOTA: renderDiagnosticoInsightsDocumentarioUnico.js v1.1
...
async function main() {
    ...
    const hub = dv.pages().where(p => p.id_midia === idMidia && p.tipo === "documentario_unico_hub")[0];
    if (!hub) { dv.paragraph(`❌ ERRO: HUB de Documentário Único com id_midia "${idMidia}" não encontrado.`); return; }
    ...
    const documentarioUnicoFolder = hub.file.folder.split('/').slice(0, -1).join('/');
    const ciclosFolder = `${documentarioUnicoFolder}/Anotações`;

    let notasDeCiclo;
    // --- CORREÇÃO DE LÓGICA APLICADA AQUI ---
    if (cicloSelecionado === "Visão Agregada (Todos os Ciclos)") {
        notasDeCiclo = dv.pages(`"${ciclosFolder}"`).where(p => p.tipo === 'documentario_unico_anotacoes' && p.hub_uid === hub.sota_uid);
    } else {
        ...
        notasDeCiclo = dv.pages(`"${ciclosFolder}"`).where(p => p.tipo === 'documentario_unico_anotacoes' && p.hub_uid === hub.sota_uid && p.ciclo === numeroCiclo);
    }

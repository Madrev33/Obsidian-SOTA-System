// Componente SOTA: renderKPIsEficienciaDocumentarioUnico.js v2.1
// Lê diretamente do raw_logs.md para máxima robustez e dados em tempo real.
// CORREÇÃO: Mensagem de erro aprimorada e verificação de tipo.
...
async function main() {
    ...
    if (!hub) { 
        dv.paragraph(`❌ ERRO: HUB de Documentário Único com id_midia "${idMidia}" não encontrado. Verifique se o tipo no frontmatter do HUB é 'documentario_unico_hub'.`); 
        return; 
    }
    
    if (!cicloSelecionado) { dv.paragraph("⚠️ Selecione um ciclo para ver os KPIs."); return; }

    const nomeDocumentarioSanitizado = hub.id_midia;
    const logBrutoPath = `99 - BACKEND/Logs_Metricas/Documentarios/${nomeDocumentarioSanitizado}/raw_logs.md`;

// Componente SOTA: renderDiagnosticoInsightsPodcastUnico.js v2.0
// Exibe os insights (ideias e reflexões) gerados durante o consumo de um podcast.
// CORREÇÃO: Remove dependência de input.dashboard e corrige lógica de busca.
...
async function main() {
    ...
    const ciclosFolder = `${podcastFolder}/Anotações`;

    let notasDeCiclo;
    if (cicloSelecionado === "Visão Agregada (Todos os Ciclos)") {
        notasDeCiclo = dv.pages(`"${ciclosFolder}"`).where(p => p.tipo === 'podcast_unico_anotacoes' && p.hub_uid === hub.sota_uid);
    } else {
        ...
        notasDeCiclo = dv.pages(`"${ciclosFolder}"`).where(p => p.tipo === 'podcast_unico_anotacoes' && p.hub_uid === hub.sota_uid && p.ciclo === numeroCiclo);
    }

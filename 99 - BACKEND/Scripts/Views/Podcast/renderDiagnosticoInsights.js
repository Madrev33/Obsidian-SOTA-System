// Componente SOTA: renderDiagnosticoInsights.js (para Podcasts Seriados) v1.1
...
async function main() {
    ...
    const podcastFolder = hub.file.folder.split('/').slice(0, -1).join('/');
    const temporadasFolder = `${podcastFolder}/Temporadas`;

    let episodios;
    // --- CORREÇÃO APLICADA AQUI ---
    if (cicloSelecionado === "Visão Agregada (Todos os Ciclos)") {
        episodios = dv.pages(`"${temporadasFolder}"`).where(p => p.tipo === 'podcast_episodio' && p.hub_uid === hub.sota_uid);
    } else {
        ...
        episodios = dv.pages(`"${cicloFolderPath}"`).where(p => p.tipo === 'podcast_episodio' && p.hub_uid === hub.sota_uid);
    }
    ...
    if (episodios.length > 0) {
        ...
        const temporadaHubs = dv.pages().where(p => p.tipo === 'podcast_temporada_hub' && p.hub_uid === hub.sota_uid);

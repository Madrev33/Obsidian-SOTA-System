// Componente SOTA: renderKPIsEficienciaPodcastSerializado.js v2.0
...
async function main() {
    ...
    // Busca pelo HUB de Podcast
    const hub = dv.pages().where(p => p.id_midia === idMidia && p.tipo === "podcast_hub")[0];
    if (!hub) { dv.paragraph(`❌ ERRO: HUB de Podcast com id_midia "${idMidia}" não encontrado.`); return; }
    ...
    if (todosOsLogs.length > 0) {
        ...
        const episodiosUnicos = new Set(logsFoco.map(l => {
            ...
            return (tempMatch && epMatch) ? `c${ciclo}-t${tempMatch[1]}-e${epMatch[1]}` : null;
        }).filter(Boolean));
        ...
        const temporadasUnicas = new Set(logsFoco.map(l => {
             ...
             const tempMatch = l.match(/\(temporada::\s*(\d+)\)/);
             const ciclo = cicloMatch ? cicloMatch[1] : '1';
             return tempMatch ? `c${ciclo}-t${tempMatch[1]}` : null;
        }).filter(Boolean));

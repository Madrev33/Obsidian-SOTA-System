// Componente SOTA: renderDiagnosticoInsights.js (para Séries) v1.1
// CORREÇÃO: Altera a busca de episódios para usar o sota_uid do HUB.
...
async function main() {
    ...
    const hub = dv.pages().where(p => p.id_midia === idMidia && p.tipo === "serie_hub")[0];
    if (!hub) { dv.paragraph(`❌ ERRO: HUB de Série com id_midia "${idMidia}" não encontrado.`); return; }
    ...
    const serieFolder = hub.file.folder.split('/').slice(0, -1).join('/');
    const temporadasFolder = `${serieFolder}/Temporadas`;

    let episodios;
    // --- CORREÇÃO APLICADA AQUI ---
    if (cicloSelecionado === "Visão Agregada (Todos os Ciclos)") {
        episodios = dv.pages(`"${temporadasFolder}"`).where(p => p.tipo === 'serie_episodio' && p.hub_uid === hub.sota_uid);
    } else {
        ...
        episodios = dv.pages(`"${cicloFolderPath}"`).where(p => p.tipo === 'serie_episodio' && p.hub_uid === hub.sota_uid && p.ciclo === numeroCiclo);
    }
    ...
    if (episodios.length > 0) {
        ...
        const temporadaHubs = dv.pages().where(p => p.tipo === 'serie_temporada_hub' && p.hub_uid === hub.sota_uid);
        ...
        const tableData = dadosOrdenados.map(p => {
            const temporadaHubCorrespondente = temporadaHubs.find(m => p.file.folder.includes(m.file.folder));
